...
**Depends on:** [Trust Domain & Attestation Policy](01-trust-domain-and-attestation-policy.md), [Legacy Integration](10-legacy-integration.md)
...
This architecture provides three independent HA budgets: the upstream management cluster has a recovery budget measured in months (the intermediate CA lifetime), downstream clusters have a recovery budget of 1 hour (the SVID TTL from the [Attestation Policy](01-trust-domain-and-attestation-policy.md)), and PostgreSQL failover targets sub-30-second recovery.

---

## 2. Design Constraints from Attestation Policy and Legacy Integration

### 2.1 Trust Domain and Nested Topology
...
The attestation policy establishes a 1-hour SVID TTL. Any downstream outage exceeding 1 hour causes workloads on that platform to lose authentication. This is the HA recovery budget for downstream clusters. The upstream's recovery budget is much longer because downstreams cache the intermediate CA and continue issuing SVIDs independently.

### 2.2 Kerberos Migration Router Placement
...
The [Legacy Integration](10-legacy-integration.md) document also establishes that the artifact signing PKI is separate from the SPIRE PKI. The signing root CA HSM, key ceremonies, and trust chain are independent workstreams that do not affect SPIRE HA design.
...
The [Legacy Integration](10-legacy-integration.md) artifact signing mechanism applies to SPIRE server binaries themselves. The SPIRE server binary must be signed by the platform team's CI/CD pipeline before deployment. The deployment automation validates the signature before writing the binary to the server node.
...
| On-prem downstream cluster failure | On-prem workloads cannot renew SVIDs. Kerberos migration router ([Legacy Integration](10-legacy-integration.md)) loses identity. Cross-protocol routing stops. | Must recover within 1 hour (SVID TTL). Patroni failover + SPIRE server restart in surviving DC. |
...
The network segmentation and isolated environment strategy (future work) may drive federation requirements for air-gapped segments. The federation design should be addressed holistically when that work is active.
...
| **Deferred** | Cloud upstream federation. Deploy federated upstream-capable servers in GCP, Azure, and AWS to eliminate on-prem network dependency for cloud downstreams. | Isolated environment strategy (future work) |
...
- `05-dns-resolution-strategy.md` — Patroni failover DNS continuity (Phase 3) depends on the Patroni topology defined here
