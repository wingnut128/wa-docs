...
|---|---|---|---|
| `tpm_devid` | **Strongest** | TPM-bound DevID certificate. The downstream server node's TPM proves possession of a key provisioned during manufacture or initial commissioning. Cannot be exported or replicated. | On-prem bare metal downstream servers with TPM 2.0 hardware. Requires TPM inventory completion ([Attestation Policy](01-trust-domain-and-attestation-policy.md) open item). |
